...
impl PlanetAI for CargonautsPlanet {
    /// Handler for the [OrchestratorToPlanet::Sunray] message, responsible for managing Sunray absorption.
    ///
    /// This method attempts to charge the planet's [EnergyCell] using the provided [Sunray].
    ///
    /// More precisely:
    /// - If the [EnergyCell] is not charged, it consumes the [Sunray] and sets the cell state to charged.
    /// - If the [EnergyCell] is already charged, the [Sunray] is ignored (wasted) and the state remains unchanged.
    ///
    /// This handler logs the reception of the sunray and the subsequent acknowledgment event,
    /// modifying the [PlanetState] in place.
    fn handle_sunray(
        &mut self,
        ...
        // Log Debug: Called handle_sunray
        LogEvent::self_directed(
    ...
    /// Handler for the [OrchestratorToPlanet::InternalStateRequest] message, responsible for exposing the internal state.
    ///
    /// This method captures a snapshot of the current [PlanetState] and converts it into a
    /// [DummyPlanetState] (a serializable representation used for visualization or debugging).
    ///
    /// More precisely, it:
    /// - Logs the receipt of the request.
    /// - Serializes the critical components of the planet (id, energy cells, rocket status, etc.).
    /// - Returns the [DummyPlanetState] to be sent back to the Orchestrator.
    fn handle_internal_state_req(
        ...
        // Log Debug: Called handle_internal_state_req
        LogEvent::self_directed(
            Participant::new(ActorType::SelfActor, state.id()),
            EventType::InternalPlanetAction,
            Channel::Debug,
...
macro_rules! generate_complex_resource {
        ($self:expr, $combinator:expr, $cell:expr, $msg:expr, { $( $complex_resource:ident ( $r1:ident, $r2:ident ) ),* $(,)? }) => {{
            paste! {
                match $msg {
                    $(
                        ComplexResourceRequest::$complex_resource($r1, $r2) => {
                            // LOG debug: "Attempting combination for {complex_resource}"
                            Self::log_internal(
                                 $self,
                                 Channel::Debug,
                                 create_payload(
                                     vec![("Debug detail".to_string(), format!("Attempting combination for {}", stringify!($complex_resource)))]
                                 )
                            );
                            $combinator
...
impl CargonautsPlanet {
    ...
    fn handle_supported_resource_request(
        ...
        //LOG trace: "SupportedResourceResponse"
        LogEvent::new(
    ...
    fn handle_supported_combination_request(
        ...
        //LOG trace: "SupportedCombinationResponse"
        LogEvent::new(
            Some(self.participant.clone()),
            Some(Participant::new(ActorType::Explorer, explorer_id)),
            EventType::MessagePlanetToExplorer,
    ...
    fn handle_generate_resource_request(
        ...
        Self::log_internal(
            ...
            create_payload(vec![
                ...
                ("state".to_string(), format!("{:?}", &state.to_dummy())),
            ]),
        );
        ...
        if is_charged {
            match req_resource {
                BasicResourceType::Carbon => {
                    ...
                    Self::log_internal(
                        ...
                        create_payload(vec![(
                            "Debug detail".to_string(),
                            "Attempting to generate Carbon".to_string(),
                        )]),
                    );
                        ...
                        Ok(r) => {
                            ...
                            Self::log_internal(
                                ...
                                create_payload(vec![(
                                    ...
                                    "Successfully generated Carbon resource".to_string(),
                                )]),
                            );
            ...
            Self::log_internal(
                ...
                create_payload(vec![(
                    "Info detail".to_string(),
                    "Uncharged energy cell. Cannot generate resource Carbon".to_string(),
                )]),
            );
        }
        //LOG trace: "GenerateResourceResponse"
        LogEvent::new(
            Some(self.participant.clone()),
            Some(Participant::new(ActorType::Explorer, explorer_id)),
            EventType::MessagePlanetToExplorer,
    ...
    fn handle_combine_resource_request(
        ...
        if !is_charged {
            ...
            Self::log_internal(
                ...
                create_payload(vec![(
                    "Debug detail".to_string(),
                    "Uncharged energy cell. Cannot combine resources".to_string(),
                )]),
            );
            let (r1, r2) = match msg {
            ...
            //LOG trace: "CombineResourceResponse"
            LogEvent::new(
                Some(self.participant.clone()),
                Some(Participant::new(ActorType::Explorer, explorer_id)),
                EventType::MessagePlanetToExplorer,
        ...
        let complex_response = generate_complex_resource!(&self, combinator, energy_cell, msg, {
            Diamond(c1, c2),
            Life(w, c),
            Water(h, o),
            Robot(s, l),
        ...
        if complex_response.is_ok() {
            ...
            Self::log_internal(
                ...
                create_payload(vec![(
                    "Info detail".to_string(),
                    "Successfully combined resources".to_string(),
                )]),
            );
        }
        //LOG trace: "CombineResourceResponse"
...
#[cfg(test)]
mod tests {
    use crate::planet_ai::{CargonautsPlanet, create_planet};
    ...
    fn test_unit_handle_supported_resource_request() {
        ...
        let participant = Participant::new(ActorType::Planet, ID::default());
        let ai = CargonautsPlanet { participant };

        let result = ai.handle_supported_resource_request(1, planet.generator());
    ...
    fn test_unit_handle_supported_combination_request() {
        ...
        let result = ai.handle_supported_combination_request(1, planet.combinator());
    ...
    fn test_unit_handle_energy_cell_request_discharge() {
        ...
        let result = ai.handle_energy_cell_request(1, planet.state());
